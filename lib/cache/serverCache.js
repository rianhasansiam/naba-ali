...
export const getCachedProducts = unstable_cache(
  async () => {
    try {
      ...
      console.error('❌ Server: Products fetch failed:', errorMessage)
      return []
    }
  },
  ['products'], // Cache key
...
export const getCachedCategories = unstable_cache(
  async () => {
    try {
      ...
      // Sanitize error message to prevent format string injection
      const errorMessage = String(error?.message || 'Unknown error').replace(/%/g, '%%')
      console.error('❌ Server: Categories fetch failed:', errorMessage)
      return []
    }
  },
  ['categories'],
...
export const getCachedReviews = unstable_cache(
  async () => {
    try {
      ...
      // Sanitize error message to prevent format string injection
      const errorMessage = String(error?.message || 'Unknown error').replace(/%/g, '%%')
      console.error('❌ Server: Reviews fetch failed:', errorMessage)
      return []
    }
  },
  ['reviews'],
...
export const getCachedUsers = unstable_cache(
  async () => {
    try {
      ...
      // Sanitize error message to prevent format string injection
      const errorMessage = String(error?.message || 'Unknown error').replace(/%/g, '%%')
      console.error('❌ Server: Users fetch failed:', errorMessage)
      return []
    }
  },
  ['users'],
...
export const getCachedOrders = unstable_cache(
  async () => {
    try {
      ...
      // Sanitize error message to prevent format string injection
      const errorMessage = String(error?.message || 'Unknown error').replace(/%/g, '%%')
      console.error('❌ Server: Orders fetch failed:', errorMessage)
      return []
    }
  },
  ['orders'],
...
export const getCachedCoupons = unstable_cache(
  async () => {
    try {
      ...
      // Sanitize error message to prevent format string injection
      const errorMessage = String(error?.message || 'Unknown error').replace(/%/g, '%%')
      console.error('❌ Server: Coupons fetch failed:', errorMessage)
      return []
    }
  },
  ['coupons'],
...
export const getCachedProduct = cache(async (productId) => {
  ...
  try {
    ...
    // Sanitize error message to prevent format string injection
    const errorMessage = String(error?.message || 'Unknown error').replace(/%/g, '%%')
    console.error(`❌ Server: Product ${productId} fetch failed:`, errorMessage)
    return null
  }
})
