...
const StatCard = ({ title, value, change, trend, icon: Icon, color = "gray" }) => (
  ...
  >
    ...
    <p className="text-2xl font-bold text-gray-900 price-number">
      {typeof value === 'number' && (title.includes('Revenue') || title.includes('Average Order')) ? `BDT ${value.toFixed(2)}` : value}
    </p>
  </motion.div>
);
...
const DashboardClient = ({ analytics }) => {
  return (
    <div className="space-y-6">
      ...
      >
        <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold mb-2">Welcome to SkyZonee Admin Dashboard</h1>
        <p className="text-gray-300 text-sm sm:text-base lg:text-lg">Here&apos;s what&apos;s happening with your fashion business today.</p>
            ...
            {analytics.monthlyData.map((data, index) => (
              <div key={data.month} className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-4 flex-1 mx-4">
                  ...
                  <span className="text-sm font-bold text-gray-900 price-number">BDT {(data.revenue / 1000).toFixed(0)}k</span>
                </div>
              </div>
            ))}
          </div>
            ...
            {analytics.topProducts.map((product, index) => (
              <div key={index} className="flex items-center space-x-4 p-3 hover:bg-gray-50 rounded-lg transition-colors">
                ...
                <div className="text-right">
                  <p className="font-bold text-gray-900 text-sm price-number">BDT {product.revenue.toLocaleString()}</p>
              ...
              <tbody>
                {analytics.recentSales.map((sale, index) => (
                  <tr key={sale.id} className="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td className="py-2 sm:py-3 px-2 sm:px-4 font-medium text-gray-900 text-sm price-number">BDT {sale.amount.toFixed(2)}</td>
